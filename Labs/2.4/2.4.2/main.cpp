...
void MergeSort(vector<int>& arr, int& check)
{
    ...
    do
    {
        ...
        if (b.empty() && d.empty() || c.empty() && b.empty() || c.empty() && d.empty()) return;
        cout <<"First stripe: ";
        Print(b);
        cout << endl;
        cout << "Second stripe: ";
...
int main() {
    ...
    switch(mode)
    {
        case 0:  for(int i = 0; i < size; ++i) arr.push_back(rand()%100);
